...
Ascii2::Ascii2(WriterFrontend* frontend) : WriterBackend(frontend)
	{
	fd = 0;
	ascii_done = false;
...
void Ascii2::InitConfigOptions()
	{
	output_to_stdout = BifConst::LogAscii2::output_to_stdout;
	include_meta = BifConst::LogAscii2::include_meta;
	use_json = BifConst::LogAscii2::use_json;
	enable_utf_8 = BifConst::LogAscii2::enable_utf_8;
	gzip_level = BifConst::LogAscii2::gzip_level;

	separator.assign(
			(const char*) BifConst::LogAscii2::separator->Bytes(),
			BifConst::LogAscii2::separator->Len()
			);

	set_separator.assign(
			(const char*) BifConst::LogAscii2::set_separator->Bytes(),
			BifConst::LogAscii2::set_separator->Len()
			);

	empty_field.assign(
			(const char*) BifConst::LogAscii2::empty_field->Bytes(),
			BifConst::LogAscii2::empty_field->Len()
			);

	unset_field.assign(
			(const char*) BifConst::LogAscii2::unset_field->Bytes(),
			BifConst::LogAscii2::unset_field->Len()
			);

	meta_prefix.assign(
			(const char*) BifConst::LogAscii2::meta_prefix->Bytes(),
			BifConst::LogAscii2::meta_prefix->Len()
			);

	ODesc tsfmt;
	BifConst::LogAscii2::json_timestamps->Describe(&tsfmt);
	...
	gzip_file_extension.assign(
		(const char*) BifConst::LogAscii2::gzip_file_extension->Bytes(),
		BifConst::LogAscii2::gzip_file_extension->Len()
		);
	}

bool Ascii2::InitFilterOptions()
	{
...
bool Ascii2::WriteHeaderField(const string& key, const string& val)
	{
...
void Ascii2::CloseFile(double t)
	{
	if ( ! fd )
		return;
...
bool Ascii2::DoInit(const WriterInfo& info, int num_fields, const Field* const * fields)
	{
	assert(! fd);
...
bool Ascii2::WriteHeader(const string& path)
	{
	if ( ! include_meta )
		return true;
...
bool Ascii2::DoFlush(double network_time)
	{
	fsync(fd);
	return true;
	}

bool Ascii2::DoFinish(double network_time)
	{
	if ( ascii_done )
		{
...
bool Ascii2::DoWrite(int num_fields, const Field* const * fields,
			     Value** vals)
	{
	if ( ! fd )
...
bool Ascii2::DoRotate(const char* rotated_path, double open, double close, bool terminating)
	{
...
bool Ascii2::DoSetBuf(bool enabled)
	{
	// Nothing to do.
	return true;
	}

bool Ascii2::DoHeartbeat(double network_time, double current_time)
	{
	// Nothing to do.
	return true;
	}

string Ascii2::LogExt()
	{
...
string Ascii2::Timestamp(double t)
	{
...
bool Ascii2::InternalWrite(int fd, const char* data, int len)
	{
	if ( ! gzfile )
	...
	while ( len > 0 )
		...
		if ( n <= 0 )
			...
			Error(Fmt("Ascii2::InternalWrite error: %s\n", err));
			return false;
			}
...
bool Ascii2::InternalClose(int fd)
	{
	if ( ! gzfile )
		{
		safe_close(fd);
	...
	switch ( res ) {
	case Z_STREAM_ERROR:
		Error("Ascii2::InternalClose gzclose error: invalid file stream");
		break;
	case Z_BUF_ERROR:
		Error("Ascii2::InternalClose gzclose error: "
		      "no compression progress possible during buffer flush");
		break;
	case Z_ERRNO:
		Error(Fmt("Ascii2::InternalClose gzclose error: %s\n", Strerror(errno)));
		break;
	default:
		Error("Ascii2::InternalClose invalid gzclose result");
		break;
	}
